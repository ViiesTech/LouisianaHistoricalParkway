...
const PriceDetails = ({ navigation }) => {
  const data = [
    { id: 1, image: images.lock, title: 'Pointe Coupee' },
    { id: 2, image: images.lock, title: 'St. Helena Parish' },
    { id: 3, image: images.lock, title: 'Avoyelles Parish' },
  ]
  return (
    <Container padding={0.01}>
      <ImageBackground source={images.forest} style={{ width: '100%', height: responsiveHeight(27) }}>
        <Header />
      </ImageBackground>
      <View style={{ backgroundColor: colors.white, flex: 1, borderTopLeftRadius: responsiveHeight(2), borderTopRightRadius: responsiveHeight(2), bottom: 10, }}>
        <ScrollView contentContainerStyle={{ flexGrow: 1, padding: responsiveHeight(2) }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <SmallContainer title="$5" icon={icons.dollar} width={26} />
          </View>
          ...
          <LineBreak val={2} />
          <ListHeading showSeeAll={false} title="Popular Places" />
          <LineBreak val={2} />
